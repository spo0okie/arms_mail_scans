...
function msg($message) {echo logdate().$message."\n";}
...
/*
 * делает SMTP запрос и ожидает ответ
 */
function smtp_talk($sock,$msg,$expect=250) {
	if (strlen($msg)) {
		fputs($sock, $msg."\r\n");
		msg('SRV < '.strtr( $msg, array(  "\n" => "\\n",  "\r" => "\\r"  )));
	}
	$answ=trim(fgets($sock,4096));
	msg("SRV > ".strtr( $answ, array(  "\n" => "\\n",  "\r" => "\\r"  )));
	...
	if ($tokens[0]==$expect)
		...
		return "ERROR ".$answ.' // <- on msg: '.$msg;
}

/*
 * Отправляет "зонд". Проверочное письмо
 */
function send_probe($server,$port,$from,$to,$auth,$probe) {
	
	//генерируем текст сообщения: три предсказания из печенья
	$msgf=popen('(/bin/fortune;/bin/fortune;/bin/fortune)|sed \'s/\n/\r\n/g\'',"r");
	$msg=fread($msgf,4096);
	pclose($msgf);
	
	if (!($sock = fsockopen($server, $port)))	return "ERROR socket_opening_error";
	stream_set_timeout ($sock,5);
	$speech=array(
			array("",220),
			array("HELO 127.0.0.1",250),
			array("AUTH PLAIN $auth",235),
			array("MAIL FROM: $from",250),
			array("RCPT TO: $to",250),
			array("DATA",354),
			//array("Subject: Probe $probe\r\n\r\n.",250),
			array("Subject: ".$probe."\r\nпроверка почты. \r\nпросьба не отвечать. \r\n\r\n.",250),
			array("QUIT",221)
	);
	foreach ($speech as $msg) {
		if (($answ=smtp_talk($sock,$msg[0],$msg[1]))!==true) {
			fclose($sock);
			return $answ;
		}
	}
...
/*
 * Читает список элементов-строк из файла
 */ 
function get_fileStrings(string $filename){
	return explode("\n",file_get_contents($filename));
}

/*
 * пишет файл из элементов-строк
 */
function set_fileStrings(string $filename, array $data){
	return file_put_contents($filename, implode("\n",$data));
}

/*
 * добавляет строку-элемент в файл
 */
function fileStringItem_add($filename,$item){
	$items=get_fileStrings($filename);
	$items[]=$item;
	set_fileStrings($filename, array_unique($items));
}

/*
 * удаляет строку-элемент из файла
 */
function fileStringItem_del($filename,$item){
	$items=get_fileStrings($filename);
	if (($idx=array_search($item,$items))===false) return false;
	unset ($items[$idx]);
	set_fileStrings($filename, array_unique($items));
}
...
/*
 * выбирает следующий СМТП сервер из пула
 */
function get_smtpServer($pool){
	global $smtp_servers;
	global $workdir;
	if (!isset($smtp_servers[$pool])) halt("Unknown SMTP pool: $pool");
	if (count($smtp_servers[$pool])) {
		$idx_file=$workdir."/smtp_pool_$pool.idx";
		//считываем индекс предыдущего сервера и выбираем следующий
		$idx=((int)file_get_contents($idx_file)+1) % count($smtp_servers[$pool]);
		//сохраняем новый индекс
		file_put_contents($idx_file, $idx);
	} else $idx=0;
	$params=['addr','port','from','auth'];
	$server=[];
	foreach ($params as $item) {
		if (!isset($smtp_servers[$pool][$idx][$item])) halt("SMTP pool item $pool / $idx / $item is unset!");
		$server[$item]=$smtp_servers[$pool][$idx][$item];
	}
	return $server;
}

/*
 * загружает настройки IMAP учетки
 */
function get_imapAcc($acc){
	global $imap_accounts;
	if (!isset($imap_accounts[$acc])) halt("Unknown IMAP account: $acc");
	$params=['imap_string','user','pass'];
	$result=[];
	foreach ($params as $item) {
		if (!isset($imap_accounts[$acc][$item])) halt("IMAP account item $acc / $item is unset!");
		$result[$item]=$imap_accounts[$acc][$item];
	}
	return $result;
}
